...
The reference smoothing factor (href) is also know as hopt, the optimal
smoothing factor, and is generally considered to perform poorly compared with
the LSCV method (Horne & Garton Smoothing Paramete for Kernel Home-Range
Analysis, Journal of Wildlife Management 70(3)

LSCV uses the the Warton or Tufto method to set the starting point but both
should converge to the same LSCV solution, so the choice is immaterial.

Parameter 1:
Point_Layer

Layer name (if in ArcMap) or path to a feature class of points (typically
animal locations). If a layer is used in ArcMap, and features are selected
in that layer, only the selected features are used, otherwise all the features
in the layer's definition query are used in the analysis. If this is a feature
...
factor for each data subset. Selecting the correct smoothing factor is key
to meaningful results. Large smoothing factors may over smooth the results,
...
pairs of points. This function is not guaranteed to work correctly if there
are duplicate locations (a warning is issued and duplicate points are offset
by a unit amount). In addition, there is no guarantee of a minimum in the
range checked (a warning is issued and hRef is used). LSCV is very slow, and
is limited to no more than 2000 points in a data subset.
...
Calculations must be done in a projected coordinate system (i..e not
...
This calculates the least squares cross validation smoothing factor for the
fixes in the shapefile
...
This prints the least squares cross validation smoothing factor for the fixes
in the FGDB
...
# approximates(?) the mean integrated square error (MISE) between the true
# density function and the kde
...
def LSCV(allDistancesSquared, n, h):
    ...
    term3b = -1.0 / (2.0 * h * h)
    term4 = n * n * h * h
    # print("term1",term1,"term2a",term2a,"term3a",term3a,"term2b",term2b,
    #       "term3b",term3b,"term4",term4)
    total = 0.0
