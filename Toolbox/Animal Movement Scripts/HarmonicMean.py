...
def HarmonicMeanDistance(points):
    ...
    for k1 in points:
        ...
        for k2 in points:
            ...
            #print i,j,d
            if d == 0:
                print "OID",k1,"and",k2,"are at the same location ("+str(x1)+","+str(y1)+") using distance of 1"
                d = 1
            z = z + 1.0/d
...
def test(n):
    ...
    print "Harmonic mean took ", ms, n, (ms * 1.0)/n, "ms/point", (1000.0* ms)/n/n, "us/point^2"
    ...
    for f in arcpy.ListFields(fc):
        if f.name == fieldName:
            if f.type == fieldType:
                ...
                raise TypeError("FieldName '" + fieldName + "' exists, but is of the wrong type");
    return False
