...
This tool is the simplified way to call utilization_smoothing,
utilization_raster, and utilization_isopleth in order. It does nothing more and
nothing less.
...
meaningful results. Large smoothing factors may over smooth the results,
adding area to the UD, and small smoothing factors may result in too much
detail, and insufficient area in the UD.
...
produce the final smoothing factor.
  ...
  * Proportion: A percentage of hRef is used. various investigators have
    suggested different percentages based on the type and distribution of the
    data under consideration.
  * LSCV: A least squares cross validation is done to select the value between
    0.05*hRef and 2.0*hRef that minimizes the LSCV score (Worton1995) between
    all pairs of points. This function is not guaranteed to work correctly if
    there are duplicate locations (a warning is issued and duplicate points are
    offset by a unit amount). In addition, there is no guarantee of a minimum
    in the range checked (a warning is issued and hRef is used). LSCV is very
    slow, and is limited to no more than 2000 points in a data subset.
  * BCV2: This is the same as LSCV, except a slightly different scoring
    function (Sain et. al. 1994) is used.
...
References:
  ...
  * Sain, S. R., K. A Baggerly, and D. W. Scott. 1994. Cross-validation of
    multivariate densities. Journal of the American Statistical Association
    89:807-817
...
class will have a field named 'contour' with the value of the isopleth, and
one or more features for each isopleth requested that exists in the input
raster. There may be multiple polylines for each isopleth. Polylines may not
close, but they should if the input is a UD Raster from the UD Raster tool.
...
UtilizationDistribution (
    Location_Layer, hRef_Method, Fixed_hRef, hRef_Modifier, hRef_Proportion,
    Isopleths, Isopleth_Lines, Isopleth_Polygons, Isopleth_Donuts,
    Output_Projection, Cell_Size
)
...
It creates the 65%, 90% UD polylines for ala single animal using the smoothing
facter as determined by LSCV

  fixes = r"C:/tmp/test.gdb/locations"
  lines = r"C:/tmp/test.gdb/contours"
  UtilizationDistribution_AnimalMovement (
    fixes, "Worton", "", "LSCV", "", "65;90", lines, "", "")
...
facter as determined by LSCV
  ...
  C:/folder> python UtilizationDistribution.py locations.shp Worton
             # LSCV # "65;90" test.gdb/contours
